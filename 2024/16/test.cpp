...
int main()
{
    ...
    for(int i = 1; i < map.map.size() - 1; i++){
        for(int j = 1; j < map.map.at(i)->size() - 1; j++){
            if(!map.map[i]->at(j)->isWall){
                std::vector<Tile*>* neighbours = map.getNeighbours(map.map[i]->at(j));
                unsigned long long size = neighbours->size()-1;
